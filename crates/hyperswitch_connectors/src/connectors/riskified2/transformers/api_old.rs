...
pub struct Riskified2RouterData<T> {
    pub amount: StringMajorUnit,
    pub router_data: T,
    amount_converter: &'static (dyn AmountConvertor<Output = StringMajorUnit> + Sync),
}

impl<T> From<(StringMajorUnit, T)> for Riskified2RouterData<T> {
...
pub struct Riskified2PaymentsCheckoutRequest {
...
impl TryFrom<&Riskified2RouterData<&FrmCheckoutRouterData>> for Riskified2PaymentsCheckoutRequest {
    ...
    fn try_from(
        ...
        let metadata: Riskified2Metadata = payment_data
            ...
            .parse_value("Riskified2 Metadata")
...
#[derive(Debug, Deserialize, Serialize, Eq, PartialEq, Clone)]
pub struct Riskified2PaymentsResponse {
    order: OrderResponse,
}
...
#[derive(Debug, Deserialize, Serialize, Eq, PartialEq, Clone)]
pub struct Riskified2FulfilmentResponse {
...
impl<F, T> TryFrom<ResponseRouterData<F, Riskified2PaymentsResponse, T, FraudCheckResponseData>>
    for RouterData<F, T, FraudCheckResponseData>
{
    type Error = Error;
    fn try_from(
        item: ResponseRouterData<F, Riskified2PaymentsResponse, T, FraudCheckResponseData>,
    ) -> Result<Self, Self::Error> {
        Ok(Self {
...
#[derive(Debug, Deserialize, Serialize, Eq, PartialEq, Clone)]
pub struct Riskified2FailedTransactionResponse {
    checkout: OrderResponse,
}

#[derive(Debug, Deserialize, Serialize, Eq, PartialEq, Clone)]
#[serde(untagged)]
pub enum Riskified2TransactionResponse {
    FailedResponse(Riskified2FailedTransactionResponse),
    SuccessResponse(Riskified2PaymentsResponse),
}

impl<F, T>
    TryFrom<ResponseRouterData<F, Riskified2FailedTransactionResponse, T, FraudCheckResponseData>>
    for RouterData<F, T, FraudCheckResponseData>
{
    type Error = Error;
    fn try_from(
        item: ResponseRouterData<F, Riskified2FailedTransactionResponse, T, FraudCheckResponseData>,
    ) -> Result<Self, Self::Error> {
        Ok(Self {
            response: Ok(FraudCheckResponseData::TransactionResponse {
...
impl TryFrom<&Riskified2RouterData<&FrmTransactionRouterData>> for TransactionSuccessRequest {
    type Error = Error;
    fn try_from(
        item_data: &Riskified2RouterData<&FrmTransactionRouterData>,
    ) -> Result<Self, Self::Error> {
...
#[derive(Debug, Deserialize, Serialize, Eq, PartialEq, Clone)]
pub struct Riskified2FulfillmentRequest {
...
impl TryFrom<&FrmFulfillmentRouterData> for Riskified2FulfillmentRequest {
    type Error = Error;
...
pub struct Riskified2WebhookBody {
    pub id: String,
    pub status: Riskified2WebhookStatus,
}

#[derive(Debug, Clone, Deserialize, Serialize)]
pub enum Riskified2WebhookStatus {
    Approved,
    Declined,
}

impl From<Riskified2WebhookStatus> for IncomingWebhookEvent {
    fn from(value: Riskified2WebhookStatus) -> Self {
        match value {
            Riskified2WebhookStatus::Declined => Self::FrmRejected,
            Riskified2WebhookStatus::Approved => Self::FrmApproved,
        }
    }
}
